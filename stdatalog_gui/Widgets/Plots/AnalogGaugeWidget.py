...
# ******************************************************************************
"""
Analog gauge widgets for rendering scalar telemetry with a dial interface.

This module provides two widgets built on top of `PlotWidget`:

- `AnalogGaugeInnerWidget`: draws a circular scale with large and small markers,
    labels, a colored pie band, and an animated needle indicating the current value.
- `AnalogGaugeWidget`: a simple container that sets up the inner gauge with a title
    label and forwards new data to it.

Responsibilities:
- Display a gauge that maps a numeric range `[min_value, max_value]` to angles.
- Render scale markers, numeric labels, current value text, and a center cap.
- Update the needle position on new data and when the widget resizes.
"""
...
class AnalogGaugeInnerWidget(PlotWidget):
    """Inner gauge widget that draws the analog dial and needle.

    Parameters
    ----------
    controller : QObject
        Controller for plot/timer integration provided by `PlotWidget`.
    comp_name : str
        Component identifier associated with the gauge telemetry.
    comp_display_name : str
        Display name shown in the parent container.
    min_value : int | float
        Minimum value shown on the scale.
    max_value : int | float
        Maximum value shown on the scale.
    left_label : str | None, optional
        Optional label displayed on the left side (passed to `PlotWidget`).
    p_id : int, optional
        Plot identifier forwarded to the base `PlotWidget`.
    ...
    -------
    valueChanged(int)
        Emitted when the displayed value changes (not used in this implementation).

    Notes
    -----
    - The widget clears inherited `PlotWidget` content while keeping its attributes,
        functions, and signals intact.
    - Scale layout and pie gradient use simple trigonometry via `math`.
    """
    ...
    def __init__(
        ...
        # Clear PlotWidget inherited graphic elements (mantaining all attributes,
    ...
    def s_is_logging(self, status: bool, interface: int):
        """Start/stop the internal timer based on logging status.
        ...
        interface : int
            Interface identifier (unused here; preserved for signature compatibility).
        """
        if status:
    ...
    def update_plot(self):
        """Update the gauge value from buffered data and request a repaint."""
        self.value = self.l_data
        self.update()

    def add_data(self, data):
        """Add new data to the gauge, clamped to the configured max.

        Parameters
        ----------
        data : int | float
            Incoming sample value to display on the gauge.
        """
    ...
    def set_scale_method(self):
        """Compute geometry-dependent parameters and build the needle polygon."""
    ...
    def create_pie(self, start, lenght, out_rad, in_rad):
        """Create a pie-shaped polygon representing the colored scale band.
        ...
        -------
        QPolygonF
            Polygon that describes the pie slice geometry for the current value.
        """
        polygon_pie = QPolygonF()
        x = y = 0
        lenght = int(
            round(
                (lenght / (self.max_value - self.min_value))
                * (self.value - self.min_value)
            )
        )
        # Outer circle line
    ...
    def draw_polygon(self, outline_pen_with=0):
        """Draw the colored scale band with optional outline."""
        ...
        colored_scale_polygon = self.create_pie(
            ...
            (
                (self.wdgt_width / 2) - (self.markers_pen.width() / 2)
            )
    ...
    def draw_large_markers(self):
        """Draw primary scale markers around the dial."""
    ...
    def draw_markers_values_text(self):
        ...
        angle_distance = (
            float(self.scale_angle_size) / float(self.scale_values_count)
        )
    ...
    def draw_small_markers(self):
        """Draw secondary (minor) scale markers between the main divisions."""
        my_painter = QPainter(self)
        my_painter.setRenderHint(QPainter.Antialiasing)
        my_painter.translate(self.width() / 2, self.height() / 2)
        ...
        steps_size = (
            ...
            / float(self.scale_values_count * self.scale_sub_values_count)
        )
        scale_line_outer_start = self.wdgt_width / 2
        scale_line_lenght = (self.wdgt_width / 2) - (self.wdgt_width / 40)
        for _ in range((self.scale_values_count * self.scale_sub_values_count) + 1):
            my_painter.drawLine(scale_line_lenght, 0, scale_line_outer_start, 0)
            my_painter.rotate(steps_size)

    def draw_values_text(self):
        """Draw the current value text near the lower center of the dial."""
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
    ...
    def draw_center_point(self, diameter=30):
        """Draw the center circle cap.

        Parameters
        ----------
        diameter : int, optional
            The diameter of the center circle in pixels. Default is 30.
        """
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
    ...
    def draw_needle(self):
        """Draw the needle polygon rotated to the current value angle."""
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        painter.translate(self.width() / 2, self.height() / 2)
        painter.setPen(Qt.NoPen)
    ...
    def resizeEvent(self, event):
        """
        Recompute scale/needle geometry when the widget size changes.
        Parameters
        ----------
        event : QResizeEvent
            The resize event triggering this method.
            (Unused but required by the Qt signature.)
        """
    ...
    def paintEvent(self, event):
        """
        Paint the complete dial: pie band, markers, labels, needle, and center.
        Parameters
        ----------
        event : QPaintEvent
...
class AnalogGaugeWidget(PlotWidget):
    """Container widget that hosts the inner analog gauge and a title label.
    ...
    comp_name : str
        Component identifier associated with the gauge telemetry.
    comp_display_name : str
        Display name shown above the gauge.
    min_value : int | float
        Minimum value shown on the scale.
    max_value : int | float
        Maximum value shown on the scale.
    ...
    def __init__(
        ...
        self.pw = AnalogGaugeInnerWidget(
            controller,
            comp_name,
            comp_display_name,
            min_value,
    ...
    def add_data(self, data):
        """Forward new data samples to the inner gauge.
